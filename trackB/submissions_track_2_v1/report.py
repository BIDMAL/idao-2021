...
def run_test(mode, dataloader, checkpoint_path, cfg):
    ...
    #        del output
    # добавил замену значений 1 и 0 в списке classification_predictions = []
    for i in range(len(classification_predictions)):
        if classification_predictions[i] == 0:
            classification_predictions[i] = 1
        else:
            classification_predictions[i] == 0
